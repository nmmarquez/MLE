...
predEconDF <- unique(select(econDF, partyid))

bind_rows(
    bind_rows(lapply(1:nDraws, function(d){
        bind_rows(lapply(1:3, function(x) mutate(predEconDF, econ3=x))) %>%
            mutate(draw=d)})) %>%
        mutate(value=c(modelPredict(modelEconFit, predEconDF, draws=nDraws))) %>%
        mutate(Model="Optim"),
    bind_rows(lapply(1:nDraws, function(d){
        bind_rows(lapply(1:3, function(x) mutate(predEconDF, econ3=x))) %>%
            mutate(draw=d)})) %>%
        mutate(value=c(polrPredict(pModel, predEconDF, draws=nDraws))) %>%
        mutate(Model="Polr")) %>%
