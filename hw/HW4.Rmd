...
```{r}
econDF <- anesDF %>%
    mutate(econ3=c(1, 1, 2, 3, 3)[econ]) %>%
    select(econ3, partyid) %>%
    na.omit
    
modelEconFit <- oprobit.fit(econ3 ~ partyid, econDF)
tibble(
...
```{r}
data(housing)
houseDF <- housing[rep(1:nrow(housing), housing$Freq),] %>%
    as_tibble %>%
    select(Sat, Infl, Cont) %>%
    mutate_all(as.numeric) %>%
    mutate(Sat=as.factor(Sat))

fhouse <- Sat ~ Infl + Cont
pModel <- polr(fhouse, data=houseDF, Hess=TRUE)
pModel$formula <- fhouse
oModel <- oprobit.fit(fhouse, houseDF)

tail(predict(pModel, newdata = houseDF, type="probs"))
tail(modelPredict(oModel, houseDF))
```