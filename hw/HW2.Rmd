...
evalPoisson <- function(y, t, lamb, negative=TRUE){
    ll <- -lamb * sum(t) + sum(y * log(t * lamb))
    if(negative){
        ll <- -ll
    }
    ll
...
DF <- tibble(x1=runif(N), x2=runif(N)) %>%
    mutate(time=sample(1:6, N, replace=T)) %>%
    mutate(y=as.numeric(rpois(N, exp(b1))))
...
```{R}
run1 <- nlminb(c(0, 0, 0), evalBetas)
run2 <- glm(y ~ 1, data=DF, family=poisson)
```